...
// Is very much the same as a single linked list, but runs two-way
// references in each node.
// Or alternatively the LinkedList can refer to a Map structure to
// hold indices of each and their next/previous nodes.
...
func (ll *LinkedList) Append(data interface{}) *LinkedList {
	if ll.firstNode == nil {
		ll.firstNode = &LinkedListNode{Value: data, PreviousNode: nil}
		ll.lastNode = ll.firstNode
		return ll
	}
	// first <-> next <-> next <-> end
	ll.lastNode.NextNode = &LinkedListNode{Value: data}
	ll.lastNode.NextNode.PreviousNode = ll.lastNode
...
func (ll *LinkedList) Prepend(data interface{}) *LinkedList {
	newFirstNode := &LinkedListNode{Value: data, NextNode: ll.firstNode, PreviousNode: nil}
